...
int main(int argc, char *argv[]) {
   ...
   // Print the created data for each process
...
#endif
    double start_create_time = MPI_Wtime();
    ...
    double create_file_time = end_time1 - start_create_time;
    ...
    //     }
    // }
    double times[6] = {end_to_end_time, create_file_time, mpi_time, create_time, enddef_time, close_time};
    char *names[6] = {"End-to-End", "File Create", "Metadata Exchange", "Metadata Consistency Check", "Metadata Write I/O (enddef)", "File Close"};
    double max_times[6];
    ...
    if (rank == 0) {
        ...
        printf("  - %-30s: %.6f\n", names[1], max_times[1]); // File Create
        printf("  - %-30s: %.6f\n", names[2], max_times[2]); // Metadata Exchange
        printf("  - %-30s: %.6f\n", names[3], max_times[3]); // Metadata consistency check
        printf("  - %-30s: %.6f\n", names[4], max_times[4]); // Metadata Write I/O
        printf("  - %-30s: %.6f\n", names[5], max_times[5]); // Close
    }
