...
static int NCOPY = 10;  // Default value
...
int main(int argc, char *argv[]) {
    ...
    printf("Reading and duplicating metadata...\n");
    if (argc < 3) {
            fprintf(stderr, "Usage: %s <source_file> <output_file> [num_copies]\n", argv[0]);
            MPI_Finalize();
            return 1;
        }
        ...
        if (argc >= 4) {
            int override = atoi(argv[3]);
            if (override > 0) {
                NCOPY = override;
                printf("Overriding NCOPY to %d\n", NCOPY);
            } else {
                fprintf(stderr, "Invalid value for num_copies. Using default NCOPY = %d\n", NCOPY);
            }
        }
