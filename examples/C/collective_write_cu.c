...
pnetcdf_io(MPI_Comm comm, char *filename, int cmode, int len)
{
    ...
    for (i=0; i<NUM_VARS; i++) {
        ...
        cudaMalloc(&dev_buf[i], bufsize * sizeof(int));
        cudaMemcpy(dev_buf[i], buf[i], bufsize * sizeof(int), cudaMemcpyHostToDevice);
    }
    ...
    MPI_Info_set(info_used, "romio_cb_write", "enable");
    err = ncmpi_create(comm, filename, cmode, info_used, &ncid);
    if (err != NC_NOERR) {
    ...
    for (i=0; i<NUM_VARS; i++) {
        err = ncmpi_put_vara_int_all(ncid, varids[i], starts, counts, dev_buf[i]);
        ERR
    }
...
int main(int argc, char **argv)
{
    ...
    verbose = 1;
    cudaError_t error = cudaSetDevice(1);
    if (error != cudaSuccess) {
        printf("Failed to set the device: %s\n", cudaGetErrorString(error));
    }
