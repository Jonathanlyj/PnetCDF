...
static int baseline_extract_meta(void *ncdp, struct hdr *file_info) {
    ...
    for (int i = 0; i < file_info->dims.ndefined; i++) {
        ...
        dim_info->name = ncp->dims.value[i]->name;
    ...
    //    strcpy(var_info->name, ncp->vars.value[i]->name);
       var_info->name = ncp->vars.value[i]->name;
       var_info->ndims = ncp->vars.value[i]->ndims;
       var_info->dimids = ncp->vars.value[i]->dimids;
...
static int add_dim(NC* ncp, int *dimidp, char* name, MPI_Offset size, size_t name_len){
    int dimid, err;
    char *nname=NULL;  /* normalized name */

    char *fname=NCI_Malloc(name_len + 1);
    strncpy(fname, name, name_len);
    fname[name_len] = '\0';
    NC_dim *dimp=NULL;
    /* create a normalized character string */
    err = ncmpii_utf8_normalize(fname, &nname);
...
int compare_dim_name(const hdr_dim *dim_a, const hdr_dim *dim_b) {
    // hdr_dim * dim_a = *(hdr_dim **)a;
    // hdr_dim * dim_b = *(hdr_dim **)b;
...
int compare_dim(const void *a, const void *b) {
    ...
    if (dim_a->name_len < dim_b->name_len) {
        ...
        strcmp_rst = strcmp(dim_a->name, dim_b->name);
        if(strcmp_rst == 0){
            if (dim_a->rank_id < dim_b->rank_id) return -1;
            if (dim_a->rank_id > dim_b->rank_id) return 1;
            if (dim_a->ranklocal_id < dim_b->ranklocal_id) return -1;
            if (dim_a->ranklocal_id > dim_b->ranklocal_id) return 1;
            return 0;
        }else {
            return strcmp_rst;
...
const NC_vararray* old_vararray, hdr_dim** sort_dims, int ndims_tmp, int** dim_sort_map, hdr_var** sort_vars, int nvars_tmp, int** var_sort_map){
    ...
    //store dims
     ...
     for (i=0; i<ndims; i++){
       ...
       if(i < old_dimarray->nread){
            ...
            if (hdr_idx == 0){
                err = add_dim(ncp, &dimid, hdr_data->dims.value[i]->name, hdr_data->dims.value[i]->size, hdr_data->dims.value[i]->name_len);
                if (err != NC_NOERR) return err;
                //update pnc header
                pncp->ndims++;
            ...
            //         goto err_check;
            //     }
            //     prev_size = ncp->dims.value[dimid]->size;
            ...
            if(dim_prev_sort_id >= 0 && compare_dim_name(hdr_data->dims.value[i], sort_dims[dim_prev_sort_id]) == 0){
                ...
                if (prev_size!= hdr_data->dims.value[i]->size){
                    ...
                    //a shread dimension, skip define since already defined
                    if (rank == hdr_idx){
                        //maintain old local id for this dim, other processes has no need to change localids at all
                        // printf("\n old_dimarray->localids[i] %d", old_dimarray->localids[i]);
                        int prev_localid = ncp->dims.localids[prev_dimid_id];
                        ncp->dims.localids[prev_dimid_id] = old_dimarray->localids[i];
                    ...
                    //update local-global index mapping (for variable)
                    new_indexes[i] = prev_dimid_id;
                    //record the dimid in sorted dim array
                    sort_dims[dim_sort_map[hdr_idx][i]]->global_id = prev_dimid_id;
                    }
                ...
                if (rank == hdr_idx){
                        //this is was from local rank, maintain its old local id for this dim
    ...
    for (i=0; i<nvars; i++){
        ...
        char *fname=NCI_Malloc(v_namelen + 1);
        strncpy(fname, hdr_data->vars.value[i]->name, v_namelen);
        fname[v_namelen] = '\0';
        /* create a normalized character string */  
        err = ncmpii_utf8_normalize(fname, &nname);
        if (err != NC_NOERR) goto err_check;
